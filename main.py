...
def death_house(house, book, dead, characters_DF):
    ...
    numero_muertos = dead.loc[(dead['house'] == house) & (
        dead['Book of Death'] == str(book))].shape[0]
...
def parser():
    ...
    parser = ArgumentParser(
        description="Porcentaje de muertos por casa en cada libro")
    parser.add_argument("house", type=str, nargs=1,
                        help='Introducir una casa de GoT')
    parser.add_argument("book", type=int, nargs=1,
                        help='Introducir el numero del libro que quieras')
...
def main():
    ...
    characters_csv['Book of Death'] = characters_csv['Book of Death'].fillna(
        0).astype(int).astype(str)
    # API characters
    characters_df = apiToDf("https://api.got.show/api/book/characters")
    characters_df = characters_df[['name', 'gender', 'house', 'alive']].rename(
        columns={'name': 'Name', 'gender': 'Gender'})
    # DF characters
    characters_DF = pd.merge_ordered(
        characters_df, characters_csv, on=['Name', 'Gender'])
    characters_DF = characters_DF.fillna('-')
    characters_DF['house'] = characters_DF['house'].replace(
        regex={r'(&apos;)': "'", r'(\[\d\])': ''})
    # DF houses
    ...
    dead = characters_DF[(characters_DF['alive'] != True) & (
        characters_DF['Book of Death'] != ('-'))]
